/*
	Decision Tree with Pessimistic Pruning
...
node* build_decision_tree(vector<vector<string> >& table,node* nodeptr,vector<vector<string> >& table_info){
	...
	if(is_homogeneous(table)){
		...
		nodeptr->total_leaves=1;
		nodeptr->total_misc=0;
		nodeptr->doprune=false;
		return nodeptr;
	}
	else if(table[0].size()==1){
		//cout<<"here "<<table.size()<<endl;
		...
		for(ii=msi.begin();ii!=msi.end();++ii){
			...
			nodeptr->total_misc+=ii->second;
			if(ii->second>nodeptr->gts_cnt){
				nodeptr->gts_cnt=ii->second;
				nodeptr->lclass=ii->first;
			}
		}
		//cout<<nodeptr->lclass<<endl;
		nodeptr->isleaf=true;
		nodeptr->label=nodeptr->lclass;
		nodeptr->total_leaves=1;
		nodeptr->total_misc-=nodeptr->gts_cnt;
		nodeptr->doprune=false;
		return nodeptr;		
	}
	else{
		...
		int mis_classifications=nodeptr->total_cnt-nodeptr->gts_cnt;
		double n1t=(double)mis_classifications+0.5;
		...
		nodeptr->total_misc=0;
		nodeptr->total_leaves=0;
		for(int i=0;i<nodeptr->children.size();++i){
			if(nodeptr->children[i]==NULL) continue;
			nodeptr->total_misc+=nodeptr->children[i]->total_misc;
			nodeptr->total_leaves+=nodeptr->children[i]->total_leaves;
		}
 
		double n1Tt=(double)nodeptr->total_misc+((double)nodeptr->total_leaves/2.0);
 
		double SE=n1Tt*((double)nodeptr->total_cnt-n1Tt);
		SE=SE/(double)nodeptr->total_cnt;
		...
		if(n1Tt+SE>n1t){
			...
			//nodeptr->total_leaves=1;
			//nodeptr->total_misc=mis_classifications;
			nodeptr->label=nodeptr->lclass;
		}
		else{
			// do not prune
...
string testDataOnDecisionTree(vector<string> &singleLine, node* nodePtr, vector<vector<string> > &tableInfo, string defaultClass){
	...
	while (!nodePtr->doprune && !nodePtr->isleaf && !nodePtr->children.empty()) {
		...
		if(childIndex==-1) return defaultClass;
